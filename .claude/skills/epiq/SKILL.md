...
- **Retrieve the ref programmatically, never derive it.** Read the `ref` field off the MCP response (`epiq_issue_list` and `epiq_board_list` include it on every entry). Do not compute it from the issue id — that it happens to be the id's last 7 characters is an implementation detail, and a hand-sliced ref that drops a character still looks plausible while matching nothing. `epiq_issue_create` returns only the `id`, so list the issue to get its ref rather than slicing.
- **Prefix commits with the ticket ref.** e.g. `61P9FS4: fix stale dots on mode switch`. This is what link-by-reference feature will match on. Copy the ref verbatim from the tool response — nothing validates a commit prefix, so a wrong one fails silently and is only noticed much later.
