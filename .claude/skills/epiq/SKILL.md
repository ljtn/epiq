...
- **Document the solution in a comment starting with `Solved by:`.** When a ticket is done, say what actually fixed it — the commit, the approach, and anything a reader would otherwise have to reverse-engineer from the diff. The fixed prefix makes solutions greppable across the board, so use it verbatim.
