...
const chart = (events: GuiEventTimelineEntry[], bounds?: [number, number]) =>
	buildFlowChart(window(events, ...(bounds ?? [])), LANES, CLOSED_LANE, TITLES);
...
describe('buildFlowChart', () => {
	...
	// A close is told by its action as well as its lane, so a board that has
	// not yet named its closed lane still draws the arrival.
	it('knows a close without the closed lane id', () => {
		...
		expect(strands.map(s => s.id)).toEqual([...LANES.map(l => l.id), 'closed']);
		expect(paths[0]!.vertices[1]!.strand).toBe(strands.length - 1);
	});
	...
	it('draws a quiet ticket under the plain view only, and only where kept', () => {
		...
		expect(
			buildFlowChart(quiet, LANES, CLOSED_LANE, TITLES, 'comments').paths,
		).toEqual([]);
		expect(
			buildFlowChart(
				quiet,
