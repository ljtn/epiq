...
export const FlowCanvas = ({
	...
	const paint = useCallback((now: number) => {
		...
		for (const {path, xs} of plottedRef.current) {
			const focused = focus !== null && path.issue === focus;

			context.globalAlpha =
				focus === null ? LINE_ALPHA : focused ? 1 : DIMMED_ALPHA;
			context.lineWidth = focused ? FOCUS_LINE_WIDTH : LINE_WIDTH;
			...
			if (path.closed) {
				...
				context.arc(
					x,
					y,
					focused ? CLOSE_DOT_RADIUS * 1.5 : CLOSE_DOT_RADIUS,
					0,
					Math.PI * 2,
				);
				context.fill();
	...
	}, [plotted, focusIssue, color, paint]);
