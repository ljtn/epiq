...
/* Link previews are fussier than browsers: LinkedIn's crawler rejects WebP
 * outright, and every platform crops to about 1.91:1. So a post's card image is
 * a JPEG sibling at that ratio, generated by `npm run blog:og`. */
...
/* The picture a link preview should use: the post's dedicated card image if one
 * has been generated, else its cover when that format is safe, else the
 * site-wide card. */
function socialImage(post) {
	const card = `${post.slug}-og.jpg`;
	if (existsSync(join(IMAGES_DIR, card))) {
		return {
			url: `${BASE_URL}/blog/images/${card}`,
			alt: post.coverAlt || post.title,
			...OG_SIZE,
		};
	}
	if (post.cover && SOCIAL_EXTS.includes(extname(post.cover).toLowerCase())) {
		return {
			url: `${BASE_URL}/blog/${post.cover.replace(/^\.\//, "")}`,
			alt: post.coverAlt || post.title,
		};
	}
	return SITE_CARD;
...
function shell({ title, description, canonical, image, type = "website", depth, head = "", body }) {
	const up = depth ? "../" : "./";
	const imageTags = [
...
function build({ quiet = false } = {}) {
	...
	// A WebP cover with no JPEG sibling means link previews fall back to the
	// generic site card, which is easy to miss until someone shares the post.
	const missing = published.filter(
		(p) => p.cover && !socialImage(p).url.includes(`${p.slug}-og.jpg`)
	);
	if (missing.length) {
		console.warn(
			`  note: no card image for ${missing.map((p) => p.slug).join(", ")} — run \`npm run blog:og\``
		);
	}
...
/* Cuts each cover to a 1200x630 JPEG that every crawler accepts. Needs
 * ImageMagick; run it after adding or replacing a cover. */
function buildSocialImages() {
	try {
		execFileSync("magick", ["-version"], { stdio: "ignore" });
	} catch {
		throw new Error(
			"`magick` (ImageMagick) is needed to generate card images: brew install imagemagick"
		);
	}
	...
	for (const post of posts) {
		...
		const src = join(IMAGES_DIR, post.cover.replace(/^\.\/images\//, ""));
		if (!existsSync(src)) continue;
		const out = join(IMAGES_DIR, `${post.slug}-og.jpg`);
		execFileSync("magick", [
			src,
			"-resize", `${OG_SIZE.width}x${OG_SIZE.height}^`,
			"-gravity", "center",
			"-extent", `${OG_SIZE.width}x${OG_SIZE.height}`,
			"-quality", "86",
			"-strip",
			out,
		]);
		console.log(`  ${post.slug}-og.jpg`);
		n++;
	}
	console.log(`generated ${n} card image${n === 1 ? "" : "s"} — rebuild to pick them up`);
}
